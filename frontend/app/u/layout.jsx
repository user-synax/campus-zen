...
export default function PublicLayout({ children }) {
  ...
  // /u/:username — minimal chrome for future profile customization
      ...
      <header className="sticky top-0 z-20 border-b border-[var(--cz-border)] bg-[var(--cz-bg)]/90 backdrop-blur">
        <div className="mx-auto max-w-[1100px] px-4 sm:px-6 h-[56px] flex items-center justify-between gap-4">
          <Link href="/" className="inline-flex items-center gap-2.5 shrink-0">
            ...
            <span className="hidden sm:inline-flex rounded-full border border-[var(--cz-border)] bg-[rgba(255,255,255,0.04)] px-2 py-0.5 text-[10px] tracking-[0.08em] uppercase text-[var(--cz-text-secondary)]">
              Public
            </span>
          </Link>
          <nav className="flex items-center gap-2">
            <Link
              href="/u"
              className="hidden sm:inline-flex text-[13px] font-medium text-[var(--cz-text-secondary)] hover:text-[var(--cz-text-primary)] px-3 py-1.5"
            >
              Explore
            </Link>
            <Link
            ...
            <Link
              href="/login"
              className="inline-flex items-center justify-center rounded-full border border-[var(--cz-border)] bg-transparent px-4 h-[36px] text-[13px] font-medium text-[var(--cz-text-primary)] hover:bg-[var(--cz-surface)] transition-colors"
            >
              Log in
            </Link>
            <Link
      ...
      <footer className="border-t border-[var(--cz-border)] mt-8">
        <div className="mx-auto max-w-[1100px] px-4 sm:px-6 h-[56px] flex items-center justify-between text-[11px] tracking-[0.06em] uppercase text-[var(--cz-text-secondary)]">
          ...
          <span className="hidden sm:inline">
            Public profiles • Clean URLs /u/:username
          </span>
          <Link
            href="/signup"
            className="text-[var(--cz-text-primary)] normal-case tracking-normal underline underline-offset-4"
          >
            Join to connect
          </Link>
        </div>
