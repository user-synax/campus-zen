...
function VerifyEmailInner() {
  ...
  const onVerify = (e) => {
    ...
    // demo: accept any 6 digits, but demo valid is 123456 for explicit success path
    setLoading(true);
    setError("");
    setTimeout(() => {
      setLoading(false);
      // for static demo accept any 6-digit as success
      setSuccess(true);
      setTimeout(() => {
        // after success push to login or onboarding
        router.push("/login");
      }, 1400);
    }, 650);
  };
  ...
  return (
    <AuthShell
      ...
      subtitle={`We sent a 6-digit code to ${masked}. Enter it below. Demo code: 123456`}
    >
      <div className="flex flex-col gap-5">
          ...
          <>
            <div className="flex items-center gap-3 rounded-[12px] border border-[var(--cz-border)] bg-[rgba(255,255,255,0.03)] px-3 py-3">
              ...
              <div className="min-w-0">
                ...
                <div className="text-[11px] tracking-wide text-[var(--cz-text-secondary)] mt-1">Code expires in 10 minutes</div>
              </div>
            ...
            <form onSubmit={onVerify} className="flex flex-col gap-4">
              <div className={shake ? "is-shaking" : ""} style={{ willChange: "transform" }}>
                {/* we reuse shake animation via inline */}
                <div className={`${shake ? "t-input is-shaking" : ""} rounded-[10px]`}>
                  <OtpInput value={otp} onChange={(v) => { setOtp(v.replace(/\D/g, "").slice(0,6)); setError(""); }} error={!!error} />
                </div>
                {error ? <p className="mt-2 text-[12px] leading-[16px] text-[var(--cz-error)]">{error}</p> : <p className="mt-2 text-[12px] leading-[16px] text-[var(--cz-text-secondary)]/70">Didn&apos;t get a code? Check spam or resend.</p>}
              </div>
        ...
        <div className="border-t border-[var(--cz-border)] pt-4 flex items-center justify-center gap-4 text-[12px]">
          <Link href="/login" className="text-[var(--cz-text-secondary)] hover:text-[var(--cz-text-primary)] underline-offset-4 hover:underline">Back to login</Link>
          <span className="h-3 w-px bg-[var(--cz-border)]" />
          <Link href="/signup" className="text-[var(--cz-text-secondary)] hover:text-[var(--cz-text-primary)] underline-offset-4 hover:underline">Create account</Link>
        </div>
      </div>
    </AuthShell>
  );
